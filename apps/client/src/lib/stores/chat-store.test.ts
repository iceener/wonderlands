...
describe('createChatStore', () => {
  ...
  test('switches threads while persisting selection and resetting thread-scoped state', async () => {
    {
      const storage = createStorage()
  ...
  test('renames and deletes the current thread while keeping local state in sync', async () => {
    {
      const store = createChatStore({
        listThreadMessages: async () => [userMessage()],
  ...
  test('edits durable user messages across draft, success, failure, and attachment-only paths', async () => {
    {
      const storage = createStorage()
      const persistedAttachment: MessageAttachment = {
        id: 'fil_edit_1',
  ...
  test('handles first-turn creation across completed, waiting, and failed boundaries', async () => {
    {
      const storage = createStorage()
...

    {
      const store = createChatStore({
        ...
        createSessionThread: async (sessionId) =>
          threadWith({ id: asThreadId('thr_1'), sessionId }),
        startThreadInteraction: async () => ({
          assistantItemId: null,
          assistantMessageId: null,
          attachedFileIds: [],
  ...
  test('forwards hydrated model and explicit reasoning selections', async () => {
    {
      ...
      const store = createChatStore({
        ...
        listThreadMessages: async () => [
          userMessage(),
          assistantMessage('Use the Google provider.'),
        ],
        storage: createStorage(),
        streamThreadEvents: async ({ onEvents }) => {
          onEvents([
  ...
  test('forwards uploaded and referenced attachments and materializes durable attachment visibility', async () => {
    {
      const storage = createStorage()
      const attachment: MessageAttachment = {
        id: 'fil_image_1',
  ...
  test('forwards explicit and default targets without leaking per-turn selection into thread state', async () => {
    {
      const interactionCalls: Array<Record<string, unknown>> = []
      const storage = createStorage()
      storage.setItem(
  ...
  test('resumes approval waits across approve, trust, rejection, and API failure outcomes', async () => {
    {
      const storage = createStorage()
      storage.setItem(
        STORAGE_KEY,
  ...
  test('materializes sandbox and execute output files as assistant attachments', async () => {
    {
