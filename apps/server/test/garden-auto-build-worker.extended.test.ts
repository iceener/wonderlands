...
test('garden auto-build worker coalesces source changes during a build without publishing the intermediate snapshot', async () => {
  const { app, config, runtime } = createTestHarness({
    AUTH_MODE: 'api_key',
    GARDEN_WORKER_AUTO_START: 'false',
    GARDEN_WORKER_DEBOUNCE_MS: '2000',
    NODE_ENV: 'test',
  })
  const admin = seedApiKeyAuth(runtime)
  let nowIso = '2026-04-03T00:00:00.000Z'
  ...
  const sourceScopeRef = join(vaultRef, 'coalescing-site')

  writeTextFile(
    join(sourceScopeRef, '_garden.yml'),
    `schema: garden/v1
title: Coalescing Garden
navigation:
  - label: Home
    path: /
public:
...
`,
  ...
  const site = gardenService.createSite(scope, {
    buildMode: 'debounced_scan',
    name: 'Coalescing Garden',
    slug: 'coalescing-garden',
    sourceScopePath: 'coalescing-site',
    status: 'active',
  })
  ...
  let changeSourceBeforeFirstBuildReturns = true
  const worker = createGardenAutoBuildWorker({
    config,
    db: runtime.db,
    gardenService: {
      publishCurrentBuild: gardenService.publishCurrentBuild,
      requestAutoBuild: async (gardenSiteId) => {
        const result = await gardenService.requestAutoBuild(gardenSiteId)

        if (changeSourceBeforeFirstBuildReturns) {
          changeSourceBeforeFirstBuildReturns = false
          writeTextFile(
            join(sourceScopeRef, 'index.md'),
            `---
title: Coalescing Garden
...
`,
  ...
  const afterIntermediateBuild = gardenService.getSiteById(scope, site.value.id)
  assert.equal(afterIntermediateBuild.ok, true)

  if (!afterIntermediateBuild.ok) {
    throw new Error(afterIntermediateBuild.error.message)
  }

  assert.notEqual(afterIntermediateBuild.value.currentBuildId, null)
  assert.equal(afterIntermediateBuild.value.currentPublishedBuildId, null)

  nowIso = '2026-04-03T00:00:04.000Z'
  assert.equal(await worker.processEligibleSites(), 0)
  nowIso = '2026-04-03T00:00:05.000Z'
  assert.equal(await worker.processEligibleSites(), 0)
  nowIso = '2026-04-03T00:00:06.000Z'
  assert.equal(await worker.processEligibleSites(), 1)
  ...
  const afterCoalescedBuild = gardenService.getSiteById(scope, site.value.id)
  assert.equal(afterCoalescedBuild.ok, true)

  if (!afterCoalescedBuild.ok) {
    throw new Error(afterCoalescedBuild.error.message)
  }

  assert.equal(afterCoalescedBuild.value.currentPublishedBuildId, builds.value[0]?.id)

  const response = await app.request('http://local/coalescing-garden')
  assert.equal(response.status, 200)
  assert.match(await response.text(), /Version 3\./)
})
