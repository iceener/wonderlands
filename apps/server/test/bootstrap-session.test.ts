...
test('bootstrap session command creates session, thread, message, run, and events in one flow', async () => {
  ...
  for (const event of createdEvents) {
    assert.deepEqual(outboxTopicsByEventId.get(event.id)?.slice().sort(), [
      'projection',
      'realtime',
    ])
  }
...
test('bootstrap session auth middleware rejects invalid API-key authentication states', async () => {
  {
...
test('bootstrap session transaction rolls back command and outbox failures', async () => {
  {
    const { app, runtime } = createTestHarness({
      AUTH_MODE: 'api_key',
      NODE_ENV: 'test',
