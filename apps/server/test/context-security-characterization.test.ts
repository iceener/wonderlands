...
test('manifest characterization rejects file bodies and data URLs while allowing summaries', () => {
  const safeManifestLikeSummary = {
    files: [
      {
        fileId: 'fil_security_image',
        mimeType: 'image/png',
        originalFilename: 'diagram.png',
        sha256: '96a296d224f285c67bee93c30f8a3091570fda78',
        sizeBytes: 4,
      },
    ],
    layers: [
  ...
  const unsafeManifestLikeSummary = {
    files: [
      {
        body: new Uint8Array([1, 2, 3]),
        dataUrl: imageDataUrl,
        storageKey: secretSentinels.storageKey,
        textContent: 'complete private file body',
      },
    ],
  }

  assert.deepEqual(scanContextSecurity(safeManifestLikeSummary, 'manifest'), [])
  assert.deepEqual(toFindingLabels(unsafeManifestLikeSummary, 'manifest'), [
    'credential_field:$.files[0].storageKey',
    'data_url:$.files[0].dataUrl',
    'file_body:$.files[0].body',
    'file_body:$.files[0].dataUrl',
    'file_body:$.files[0].textContent',
  ])

  // Context Assembly v2 manifest types/generators do not exist at this baseline. Once added,
  // pass the generated manifest to the manifest scan above rather than duplicating its shape here.
})
