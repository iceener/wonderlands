...
import type {
  AgentRecord,
  AgentRepository,
  AssignAgentActiveRevisionInput,
  CreateAgentInput,
  UpdateAgentDefinitionInput,
  UpdateAgentStatusInput,
} from '../../../../domain/agents/agent-repository'
...
export const createAgentRepository = (db: RepositoryDatabase): AgentRepository => {
    ...
    getBySlug: (scope: TenantScope, slug: string): Result<AgentRecord, DomainError> => {
      const rows = db
        ...
        .where(
          and(
            eq(agents.slug, slug),
            eq(agents.tenantId, scope.tenantId),
      ...
      if (rows.length > 1) {
        return err({
          message: `agent slug ${slug} resolves to multiple live agents in tenant ${scope.tenantId}`,
          type: 'conflict',
        })
      }

      return ok(toAgentRecord(rows[0]))
    },
