...
export const pruneThreadHistoryInTransaction = (
  ...
  try {
    ...
    if (sandboxExecutionIds.length > 0) {
      tx.delete(sandboxWritebackOperations)
        .where(
          and(
      ...
      tx.delete(sandboxExecutionFiles)
        .where(
          and(
      ...
      tx.delete(sandboxExecutionPackages)
        .where(
          and(
    ...
    if (kernelSessionIds.length > 0) {
      tx.delete(kernelSessionArtifacts)
        .where(
          and(
